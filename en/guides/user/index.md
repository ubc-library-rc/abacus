...
> Searching for phrases must be done using *double* quotation marks. Using single quotation marks is equivalent to using *no* quotation marks. Thus, `'income inequality'` produces the same results as `income inequality`: a search for objects containing either the term "income" or "inequality".
